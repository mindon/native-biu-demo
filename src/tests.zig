...
test "clicking the buttons drives the model through typed dispatch" {
    ...
    const open_hello = try expectByText(tree.root, .button, "Open local index.html");
    main.update(&model, tree.msgForPointer(open_hello.id, .up).?);
    try testing.expect(model.open_hello_requested);
    try testing.expectEqualStrings("zero://app/index.html", model.hello_url);

    const hello_url = main.takeHelloWindowUrl(&model);
    try testing.expectEqualStrings("zero://app/index.html", hello_url);
    try testing.expect(!model.open_hello_requested);
    try testing.expectEqualStrings("", model.hello_url);
}

test "url window derives title and source from url" {
    try testing.expectEqualStrings("Hello", web.extractHtmlTitle("<html><head><title>Hello</title></head></html>").?);
    try testing.expectEqualStrings("Spaced", web.extractHtmlTitle("<TITLE data-x=\"1\">  Spaced  </TITLE>").?);
    try testing.expect(web.extractHtmlTitle("<html><head></head></html>") == null);

    const source = web.sourceForUrl("zero://app/panel.html");
    try testing.expectEqual(native_sdk.WebViewSourceKind.assets, source.kind);
    try testing.expectEqualStrings("frontend/dist", source.asset_options.?.root_path);
    try testing.expectEqualStrings("panel.html", source.asset_options.?.entry);

    const remote = web.sourceForUrl("https://example.com");
    try testing.expectEqual(native_sdk.WebViewSourceKind.url, remote.kind);
    try testing.expectEqualStrings("https://example.com", remote.bytes);
}
